...
一个面向中文读者的静态优先多页 Web 应用。它把具身智能（VLA）、世界模型、目标检测、视觉表征、分割与机器人多模态模型放在同一套可检索坐标系中，并配有本地论文库、正式发表追踪与领域趋势解读。
...
便携 ZIP 必须先完整解压，再双击 `启动图鉴.bat`。如果接收电脑已经安装、登录并至少启动过一次 Codex Desktop，可再双击 `安装每日优化循环.bat`；安装器会根据实际解压目录创建每天北京时间 04:00 的项目自动任务。完整步骤与能力边界见 `迁移到新电脑.md`。

ZIP 不包含原电脑的登录状态、API key、浏览器会话、模型权限或额度。电脑关机、Codex 未运行、断网或额度不足时，网页本身不能在后台修改项目；接收者仍需运行一次安装器、重启 Codex，并在“自动化”页确认任务启用。

### Linux：安装完整无人值守链路

完整的“维护 → 独立门禁 → GitHub → Cloudflare → 公网探测 → Outlook 汇报”链路由仓库内的 systemd 包装器负责。首次安装：
...
定时器直接使用 `Asia/Shanghai`：每日维护固定在北京时间 04:00，公开全文续传在 05:30，并通过同一把锁避免并发。`loginctl ... enable-linger` 使用户退出图形会话后仍可触发用户级定时器；若系统策略拒绝该命令，需要管理员启用。Windows 的便携 Codex 任务负责维护、验证和报告准备，但不等同于此 Linux 包装器的自动 GitHub/Cloudflare/Outlook 发布闭环；要在 Windows 达到同等能力，应在 WSL/Linux 中安装上述链路或另行提供经过验证的等价包装器。

Linux 包装器固定使用当前账户可用的最高档 `gpt-5.6-sol` 与 `xhigh`，并从 Codex 运行头核验模型、推理档、沙箱和审批策略；任一项不符都禁止发布。维护代理不能修改包装器、便携提示词、门禁、部署脚本、systemd 模板或锁定依赖，`node_modules` 与本地 Git 配置也在代理前后做内容摘要核验，提交/推送禁用本地 Git hooks。只有显式标记且发生在代理启动前的锁冲突或临时网络错误会以 75 进入 systemd 的有限重试；代理启动后的错误、脏树、门禁失败、秘密扫描、GitHub、Cloudflare 或 Outlook 失败都不会从头盲目重跑。失败改动保留为本机 0600 日志与补丁供人工恢复，不自动回滚可能已发生的外部发布。

发布成功不能只依赖模型自述：包装器会重跑数据、Python、18 项学术候选、全部 JavaScript、PDF 策略、24 张架构几何和 42 次 Chromium 交互门禁；提交树必须与验收树相同。Cloudflare 回执必须出现本轮之前不存在的新 deployment ID，且为 `Production/main`、Source 对应 GitHub commit；随后核对 canonical 数据哈希、19 个公网入口、全部正式开放 PDF 的 `206`/`Content-Range`/`%PDF-`，并实际渲染一篇 Springer 与一篇 CVF 论文。Outlook 账号、收件配置和有效运行档位在发信前后复核；邮件失败不会抹掉已经发生的 GitHub/Cloudflare 事实，但会把整条自动链标为“未完整成功”。
...
| `radar.html` | 论文雷达：正式期刊/会议 + 本地 arXiv 导读库 + OpenAlex / Hugging Face Papers 实时发现、来源状态与去重 |
| `reader.html?id=...` / `reader.html?paper=...` | 站内论文阅读器：本地合法缓存优先，云端通过公开源白名单流式阅读；同时呈现初学者解析、断言级证据与方法流程图 |
...
.
├─ assets/
│  ├─ css/main.css          # 深色空间玻璃首页 + 暖色学术玻璃内页、响应式与可访问动效
│  ├─ vendor/pdfjs/         # 固定版本的 PDF.js 运行文件与许可证
│  └─ js/
│     ├─ app.js             # 页面入口与按需加载
...
│  ├─ fetch_fulltext.py     # 只下载许可边界明确的公开全文到被 Git 忽略的 library/
│  ├─ check_site_browser.mjs# 11 页 + 三个附加视图、三档视口的 Chromium 门禁
│  ├─ run_daily_maintenance.sh # 失败闭锁的维护、发布、公网探测与邮件包装器
│  ├─ deploy_pages.sh       # 经门禁后发布到 Cloudflare Pages
...
├─ functions/api/pdf.js     # 严格公开源白名单的 Cloudflare PDF Range 流
├─ config/systemd/          # 北京时间定时器与共享锁服务模板
├─ .codex/
...
论文雷达不再由浏览器直连 Semantic Scholar；每日维护把可核验的引用快照写入静态数据，避免公共接口的 CORS 与限流导致页面噪声。模型详情页仍保留由用户主动点击触发的单篇引用查询，失败时静默隐藏且不影响正文。
...
> 2026-07-19 实测：OpenAlex 与 Hugging Face Papers 支持浏览器跨域读取；Semantic Scholar、arXiv Atom、CVF 与 OpenReview 的 CORS 并不一致。前端不绕过来源策略：arXiv 与 Semantic Scholar 由每日维护生成静态快照；站内阅读优先使用本机合法缓存，其次在 Cloudflare 上经严格域名/路径白名单流式转发公开 PDF，最后退回原始来源页。代理不接受任意 URL，也不抓取订阅墙内容。
...
建议每天运行一次学术候选更新；默认回溯 30 天，适合与每日北京时间 04:00 维护循环同频，无需高频轮询：
...
1. 保持现有“**深色空间玻璃舞台 + 暖白学术正文**”双层视觉系统：深色用于首页沉浸入口与论文阅读环境，暖白用于高密度研究内容；不做廉价霓虹科技风。保留用户已有改动，不做破坏性 Git 操作。无人值守包装器只在独立门禁通过后负责自动提交和推送，维护代理本身不得提交。
...
在 `radar.html` 至少验证一个预置方向、一个自定义关键词和“正式期刊 / 会议”来源：标题、作者、日期、摘要折叠、正式版本/arXiv 原始来源、站内阅读全文与 GitHub 搜索按钮按实际可用性出现；本地库和正式记录先显示，OpenAlex / Hugging Face Papers 独立更新，来源标签、冲突提示、1 小时新缓存与 24 小时旧缓存标识正确。某一来源失败时，本地库和其他来源仍可用，错误态与重试可操作。付费墙记录只能显示元数据和来源页，不得伪装为开放全文。
...
站点 JavaScript 当前共 13 个文件，另有 1 个架构几何检查脚本；门禁必须动态枚举全部文件，不把数量写死，并执行几何检查：
...
在已执行 `npm ci` 和 `npx playwright install chromium` 的 Linux/CI 环境，还必须运行真实交互与边缘函数门禁：

```bash
npm run test:function
npx --no-install wrangler pages functions build functions --outfile /tmp/atlas-pages-worker.js
# 先在 127.0.0.1:8000 启动站点
ATLAS_BASE_URL=http://127.0.0.1:8000 npm run test:browser
```

浏览器脚本固定产生 42 次渲染，并实际操作搜索、筛选/重置、收藏往返、三模型对比、Radar 搜索、Reader 缩放与解析展开、谱系缩放/拖拽/分类、时间线和术语搜索；只有每项交互、控制台、请求、溢出和触控尺寸同时通过才返回成功。

Windows 上如果 `node` 不在 `PATH`，可使用 Codex 自带运行时中实际存在的 `node.exe`；必须记录最终使用的路径。找不到运行时就把 JS 语法检查列为未通过，不能因为命令没运行而谎报通过。还要检查 HTML 本地引用、正式 URL 与 `/api/pdf` 白名单流式路由无 404、无 CDN/UI 框架运行时依赖、运行时代码没有已移除的学习页、Quiz 或闯关残留，并逐页核对 A 级代码行号/复制反馈。任何数据、Python、JavaScript、架构几何或浏览器控制台门禁失败，都要修复后重跑；实体手机验收按周期执行，未在当轮完成时必须如实保留 pending，但三档真实 Chromium 验收通过后不阻断安全的日常内容发布。
...
9. 11 个正式页面在 1280 / 390 / 320 的溢出和控制台结果、学术追踪三视图、正式论文视图，以及实体手机最近一次访问结果；
10. `build_index.py`、`validate_data.py`、Python 编译、模型候选桥接回归、学术候选回归、全部站点 JavaScript、42 个浏览器渲染与几何脚本的原始摘要；
11. 六类谱系逐类节点/边/重叠/穿线/交叉数字、11 页逐页三档结果、术语社区证据覆盖数字，以及当天未解决的阻塞和下一轮第一优先级。

不允许只写“已优化”“全部正常”而不附上述数字和可复核证据。

每轮在最终发布前还必须做一次“网站增量机会审查”，分别列出可立即安全完善、需要更多一手证据、以及新颖但应小范围验证的想法；逐项说明初学者价值、证据、风格与 11 页边界兼容性、维护成本、风险和优先级。低风险且可逆的改进可以当轮实现并复测，其余进入 pending，不得为了新颖恢复学习页、Quiz 或堆砌功能。若配置了授权邮件通道，最终中文报告发送到指定收件人；邮件失败保留本地报告，不推翻已经验证的数据与外部发布事实，但整条无人值守链必须标为未完整成功，不能写成 PASS。
