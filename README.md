...
每轮在最终发布前还必须做一次“网站增量机会审查”，分别列出可立即安全完善、需要更多一手证据、以及新颖但应小范围验证的想法；逐项说明初学者价值、证据、风格与 11 页边界兼容性、维护成本、风险和优先级。低风险且可逆的改进可以当轮实现并复测，其余进入 pending，不得为了新颖恢复学习页、Quiz 或堆砌功能。若配置了授权邮件通道，最终中文报告发送到指定收件人；邮件失败只记录错误并保留本地报告，不影响事实状态。
